...
export const getAllUsers = async (req, res) => {
  try {
    ...
    const userOrgs = await UserOrgRepository.getByField("email", req.user.email);
    const isAdmin = userOrgs.some(org => 
      org.role === roles.ADMIN || org.role === roles.SUPER_ADMIN
    );
    ...
    const users = await UserRepository.getAll(); // This will scan the entire table
